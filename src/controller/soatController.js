...
import soat from '../models/soat.js'; // Importamos el modelo Item

const router = express.Router();


// Obtener los ítems creados por el usuario autenticado
export const getSoat = async (req, res) => {
    try {
      const userId = req.userId; // El ID del usuario viene del token JWT
      const soat = await soat.find({ user: userId }); // Filtramos los ítems por el usuario
  
      res.status(200).json(soat); // Respondemos con los ítems encontrados
    } catch (error) {
...
export const crearSoat = async (req, res) => {
    try {
